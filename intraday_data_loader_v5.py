"""
intraday_data_loader_v5.py – Korrigierter Intraday-Daten-Loader mit Zeitzonen-Behandlung
"""
...
def load_intraday_data(symbols=['SPY', '^VIX'], period='2y', interval='60m'):
    ...
    for symbol in symbols:
        ...
        try:
            ...
            # Zeitzone entfernen (auf naive umstellen)
...
def load_vix3m_proxy():
    ...
    # Zeitzone auf naive umstellen (falls vorhanden)
    if df.index.tz is not None:
        df.index = df.index.tz_localize(None)
...
def main():
    ...
    if vix3m.empty:
        ...
        # Zeitzone des Intraday-Index auf naive umstellen (falls vorhanden)
        idx_naive = df.index.copy()
        if idx_naive.tz is not None:
            idx_naive = idx_naive.tz_localize(None)
        
        # Für jeden Intraday-Zeitpunkt den Tageswert holen
        for i, idx in enumerate(df.index):
            date_only = idx_naive[i].normalize()
            
            # Suche den Wert für dieses Datum
            if date_only in vix3m.index:
                vix3m_intraday[idx] = vix3m.loc[date_only]
            else:
                # Nächsten verfügbaren Tag suchen (maximal 5 Tage rückwärts)
                found = False
                for days_back in range(1, 6):
                    check_date = date_only - pd.Timedelta(days=days_back)
                    if check_date in vix3m.index:
                        vix3m_intraday[idx] = vix3m.loc[check_date]
                        found = True
                        break
                if not found:
                    # Fallback: asof mit tz-naive
