"""
intraday_data_loader_fixed.py – Korrigierter Intraday-Daten-Loader
"""
...
def load_intraday_data(symbols=['SPY', '^VIX'], period='2y', interval='60m'):
    ...
    """
    print("=" * 60)
    print("📊 LADE INTRADAY-DATEN (FIXED)")
    print("=" * 60)
    ...
    for symbol in symbols:
        ...
        try:
            ...
            # Nur Close-Preis behalten für einfachere Verarbeitung
            df = df[['Close']].copy()
            df.columns = [symbol.replace('^', '')]
            
            # Zeitstempel auf volle Stunde normalisieren (für besseres Matching)
    ...
    # Alle Daten zusammenführen (outer join, dann ffill für VIX)
    df_combined = pd.DataFrame(index=dfs[list(dfs.keys())[0]].index)
    
    for symbol, df in dfs.items():
        # Für jeden Zeitstempel den nächsten verfügbaren Wert nehmen
        df_combined[symbol.replace('^', '')] = df.reindex(df_combined.index, method='ffill')
