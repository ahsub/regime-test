"""
test_ad_ratio.py – Test Advance-Decline Ratio statt AD-Linie
=============================================================

Dieses Skript testet, ob die Advance-Decline Ratio (statt der AD-Linie)
die 2022er-Fehlklassifikationen beheben kann.
"""
...
def load_data():
    ...
    # Advance-Decline Ratio (NYA als Proxy für Marktbreite)
    try:
    ...
    # AD-Ratio berechnen: (heutiger Schluss - Vortagesschluss) / Vortagesschluss
    # Als Proxy für Advances/Declines verwenden wir die prozentuale Veränderung der Marktbreite
...
# =============================================================================
# 3. Erweiterte Logik mit AD-Ratio
# =============================================================================

def classify_regime_v3_ad_ratio(vix, vix3m, gex, ad_ratio, distribution_days,
                                ad_ratio_threshold=-0.01, dd_threshold=3):
    """
    Erweiterte Logik: AD-Ratio statt AD-Linie.
    """
    # 1. Original-Logik
    regime = classify_regime_v2(vix, vix3m, gex)
    
    # 2. Nur bei NEUTRAL oder POST_PANIC_REVERSION: Prüfen
    if regime in ["NEUTRAL", "POST_PANIC_REVERSION"]:
        # AD-Ratio: Wenn negativ (mehr Declines als Advances)
    ...
    for i in range(len(df)):
        ...
        if kwargs:
            regime = regime_func(
                ...
                row['ad_ratio'], row['distribution_days_5'],
                **kwargs
            )
        else:
...
def analyze_2022_miss(df, v2_signals, v3_signals):
    ...
    print("\n" + "=" * 60)
    print("📊 ANALYSE DER 2022ER-FEHLKLASSIFIKATIONEN (AD-RATIO)")
    print("=" * 60)
...
def main():
    print("=" * 60)
    print("📈 TESTE AD-RATIO (statt AD-Linie)")
    print("=" * 60)
    ...
    # V3 mit AD-Ratio (verschiedene Schwellwerte)
    print("🔧 Führe V3-Backtest durch (AD-Ratio, threshold=-0.01)...")
    v3_result_01 = backtest_strategy(
        df, classify_regime_v3_ad_ratio, "V3 AD-Ratio (-0.01)",
        ad_ratio_threshold=-0.01, dd_threshold=3
    )
    
    print("🔧 Führe V3-Backtest durch (AD-Ratio, threshold=-0.005)...")
    v3_result_005 = backtest_strategy(
        df, classify_regime_v3_ad_ratio, "V3 AD-Ratio (-0.005)",
        ad_ratio_threshold=-0.005, dd_threshold=3
    )
    
    # Vergleich
    print("\n" + "=" * 60)
    print("📊 VERGLEICH V2 vs. V3 (AD-RATIO)")
    print("=" * 60)
    
    print(f"\n{'Kennzahl':<20} | {'V2':<15} | {'V3 (thr=-0.01)':<18} | {'V3 (thr=-0.005)':<18}")
    print("-" * 80)
    print(f"{'Sharpe Ratio':<20} | {v2_result['sharpe_ratio']:>14.2f} | {v3_result_01['sharpe_ratio']:>17.2f} | {v3_result_005['sharpe_ratio']:>17.2f}")
    print(f"{'Gesamtrendite':<20} | {v2_result['total_return']:>14.2%} | {v3_result_01['total_return']:>17.2%} | {v3_result_005['total_return']:>17.2%}")
    
    print(f"\n📊 Regime-Verteilung V2:")
    for regime, count in v2_result['regime_counts'].items():
        print(f"   {regime}: {count} ({count/len(df)*100:.1f}%)")
    
    print(f"\n📊 Regime-Verteilung V3 (AD-Ratio -0.01):")
    for regime, count in v3_result_01['regime_counts'].items():
        print(f"   {regime}: {count} ({count/len(df)*100:.1f}%)")
    
    print(f"\n📊 Regime-Verteilung V3 (AD-Ratio -0.005):")
    for regime, count in v3_result_005['regime_counts'].items():
        print(f"   {regime}: {count} ({count/len(df)*100:.1f}%)")
    
    # Analyse der 2022er-Fehltage
    analyze_2022_miss(df, v2_result['signals'], v3_result_01['signals'])
