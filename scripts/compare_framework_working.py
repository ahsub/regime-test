...
try:
    ...
    print("✅ Framework-Modelle erfolgreich geladen")
...
def compare_models(symbol: str = "SPY"):
    ...
    # 2. Ihr Modell (K-Means)
    print("\n" + "=" * 40)
    print("IHR MODELL (K-Means)")
    print("=" * 40)
    ...
    if FRAMEWORK_AVAILABLE:
        ...
        # HDBSCAN nur wenn verfügbar
        try:
            models['HDBSCAN'] = HDBSCANModel(min_cluster_size=10)
        except:
            print("  ⚠️ HDBSCAN nicht verfügbar, überspringe...")
        
        for name, model in models.items():
            print(f"  - {name}...")
            try:
                # Korrekte Methode: fit + predict
                model.fit(X_scaled)  # oder X, je nachdem was das Framework erwartet
                labels = model.predict(X_scaled)
                framework_results[name] = labels
                n_regimes = len(np.unique(labels))
                print(f"    ✅ {n_regimes} Regime gefunden")
                print(f"    Verteilung: {np.bincount(labels.astype(int))}")
    ...
    if FRAMEWORK_AVAILABLE:
        ...
        for name, labels in framework_results.items():
            if labels is not None and len(labels) == len(your_regimes):
                ...
                print(f"  {name:15}: ⚠️ Nicht vergleichbar (len={len(labels) if labels is not None else 'None'})")
    else:
    ...
    with open(results_dir / 'comparison_results_working.pkl', 'wb') as f:
        pickle.dump(results, f)
    print(f"\n✅ Ergebnisse gespeichert in results/comparison_results_working.pkl")
    ...
    try:
        ...
        # Framework Vergleich (max 2 Plots)
        idx = 0
        for name, labels in framework_results.items():
            if labels is not None and idx < 2:
                axes[1, idx].bar(range(len(labels)), labels, alpha=0.5, width=1)
                axes[1, idx].set_title(f'{name} Regime')
                axes[1, idx].set_ylabel('Regime')
                idx += 1
        
        plt.tight_layout()
        plt.savefig(results_dir / 'comparison_visualization_working.png', dpi=150)
        print("📈 Visualisierung: results/comparison_visualization_working.png")
        plt.close()
    ...
    if comparison:
        ...
        print(f"  🏆 Beste Übereinstimmung: {best_model}")
        print(f"     ARI={comparison[best_model]['ARI']:.3f}")
        print(f"     V-Measure={comparison[best_model]['V-Measure']:.3f}")
        print(f"\n  💡 Interpretation:")
        print(f"     - ARI > 0.5: Gute Übereinstimmung")
        print(f"     - ARI > 0.7: Sehr gute Übereinstimmung")
        print(f"     - ARI < 0.3: Geringe Übereinstimmung")
    else:
        print("  ⚠️ Keine Framework-Ergebnisse zum Vergleichen")
        print("  💡 Versuchen Sie, die Modelle direkt zu testen:")
        print("     python3 -c 'from ml_service.models.kmeans import KMeansModel; print(\"OK\")'")
