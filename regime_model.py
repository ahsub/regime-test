...
def fit_markov_model(returns: pd.Series, n_regimes: int = 3) -> object:
    ...
    ----------
    returns : pd.Series
        Zeitreihe der Renditen
    n_regimes : int
        Anzahl der versteckten Zustände (3 oder 4)
    
    Rückgabe:
    --------
    object : Das gefittete Modell
    """
    # Modell definieren
...
def interpret_regimes(res: object) -> pd.DataFrame:
    ...
    means = res.params[:n]
    stds = np.sqrt(res.params[n:2*n])
    
    # Regime-Daten sammeln
    regime_data = []
    for i in range(n):
        regime_data.append({
            'regime': i,
...
def plot_regime_transitions(probabilities: np.ndarray, 
    ...
    """
    # Dominantes Regime für jeden Zeitpunkt
...
def main():
    ...
    # 2. Renditen berechnen (z.B. aus VIX oder S&P 500)
    # Für den Anfang: Wir verwenden die VIX-Renditen als Proxy für Marktstimmung
    # Später können wir S&P 500-Renditen verwenden
